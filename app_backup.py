...
with col3:
    ...
    st.metric("Avg confidence", f"{avg_conf:.1%}",
              help="Average confidence score across all detections. Higher means BirdNET is more sure of its predictions")
with col4:
    if len(filtered) > 0:
        top = filtered.loc[filtered["Confidence"].idxmax()]
        st.metric("Top species", top["Common name"],
                  help="The species detected with the highest confidence score in this recording")
    else:
...
if nt_model is not None and os.path.exists(selected_file):
    ...
    if len(nt_results) > 0:
        # Get top-1 predictions per segment
        nt_top1 = nt_results[nt_results["Rank"] == 1].copy()
        nt_top1_filtered = nt_top1[nt_top1["Confidence"] >= min_confidence]

        # --- Side-by-side top prediction summary ---
        col_nt, col_bn = st.columns(2)

        with col_nt:
            st.markdown("#### 🇦🇺 Custom NT Model")
            if len(nt_top1) > 0:
                best_nt = nt_top1.loc[nt_top1["Confidence"].idxmax()]
                st.metric("Top prediction", best_nt["Species"])
                st.metric("Confidence", f"{best_nt['Confidence']:.1%}")
                st.metric("Segments analysed", len(nt_top1))

                # Check against ground truth
                basename = os.path.basename(selected_file)
                for key, label in ground_truth.items():
                    if key in basename:
                        species_name = label.split(" (*")[0]
                        if species_name.lower() in best_nt["Species"].lower() or best_nt["Species"].lower() in species_name.lower():
                            st.success(f"✅ Correctly identified as **{best_nt['Species']}**")
                        else:
                            st.warning(f"Predicted **{best_nt['Species']}** (actual: {species_name})")
                        break
            else:
        ...
        with col_bn:
            ...
            if len(filtered) > 0:
                best_bn = filtered.loc[filtered["Confidence"].idxmax()]
                st.metric("Top prediction", best_bn["Common name"])
                st.metric("Confidence", f"{best_bn['Confidence']:.1%}")
                st.metric("Detections", len(filtered))

                basename = os.path.basename(selected_file)
                for key, label in ground_truth.items():
                    if key in basename:
                        species_name = label.split(" (*")[0]
                        if key.replace("_", " ").lower() in best_bn["Common name"].lower():
                            st.success(f"✅ Correctly identified as **{best_bn['Common name']}**")
                        else:
                            st.error(f"⚠️ Misidentified as **{best_bn['Common name']}**")
                        break
            else:
